...
if ( ! class_exists( 'WP_Fail2Ban_Redux' ) ) {
  ...
  class WP_Fail2Ban_Redux {
    ...
    public function is_post_to_login(){
    ...
    public function do_login_rate_limiting($args=null){
      $this->loginPostFreq = new WP_Fail2Ban_Redux_User_Frequency(
        'login-post',
        function(){
          return $this->is_post_to_login();
        });
      $ip = $_SERVER['REMOTE_ADDR'];
      if($this->loginPostFreq->is_over_threshold($ip)){
	$this->logger->openlog( 'ratelimiting' );
	$this->logger->syslog( "Rate limiting post to /wp-login.php" );
	$this->logger->_exit( 'ratelimiting' );
    ...
    public function is_invalid_request(){
      ...
      if (preg_match('/\/wp-content\/uploads\/.*\.php/i', $url)
          ...
          || preg_match('/\.svn\/|\.git\//i', $url)
      ){
        return true;
      }else if(strtoupper($_SERVER['REQUEST_METHOD']) == 'POST'
               && preg_match('/\/wp-content\//i', $url)){
        return true;
      }
      return false;
    }

    public function check_for_invalid_requests($args=null){
      $url = $_SERVER['REQUEST_URI'];
      $ip = $_SERVER['REMOTE_ADDR'];
      if($this->is_invalid_request()){
	$this->logger->openlog( 'invalidrequest' );
	$this->logger->syslog( "Invalid request to $url" );
	$this->logger->_exit( 'invalidrequest' );
